    ...
    <>
      ...
      <RefTable
        ...
        rows={[
          [
            "Docker",
            "Running locally — Docker Desktop, Colima, Rancher Desktop, or Podman with a Docker-compatible socket. Fin auto-detects the common socket paths.",
          ],
          [
            "git",
            <>
              Only used by <Code>fin plugs install &lt;git-url&gt;</Code>.
              Optional — catalog installs like{" "}
              <Code>fin plugs install laravel</Code> fetch over plain HTTPS and
              need no git.
            </>,
          ],
          [
            "Python 3.11+",
            <>
              <strong>Only</strong> for installing from source. The prebuilt
              binary needs no Python, pip, or virtualenv.
            </>,
          ],
        ]}
      />
      ...
      <RefTable
        ...
        rows={[
          [
            "1",
            <>
              Detects your OS/arch and downloads the matching release tarball{" "}
              <Code>fin-&lt;os&gt;-&lt;arch&gt;.tar.gz</Code> (os ∈{" "}
              <Code>macos</Code>/<Code>linux</Code>, arch ∈ <Code>arm64</Code>/
              <Code>x64</Code>) from the GitHub Releases of{" "}
              <Code>sharanvelu/fin</Code>.
            </>,
          ],
          [
            "2",
            <>
              Unpacks it into <Code>~/.local/lib/fin-cli</Code> (created if
              missing; override with <Code>FIN_HOME_DIR</Code>), stripping the
              tarball&apos;s top-level <Code>fin/</Code> directory so the
              install dir is the package root: the <Code>fin</Code> executable
              with its <Code>_internal/</Code> runtime alongside. Entirely
              user-local — never uses <Code>sudo</Code>.
            </>,
          ],
          [
            "3",
          ...
          [
            "5",
            <>
              Runs <Code>fin --version</Code> once — the first launch of the
              unsigned binary is slow (~15s while the OS verifies it), so the
              installer pays that cost up front and your first real command
              starts instantly.
            </>,
          ],
          [
            "6",
            <>
              Creates the plugs directory at <Code>~/.fin/plugs</Code> (override
              with <Code>FIN_DATA_DIR</Code>). Plugs themselves are not bundled
              — install them with <Code>fin plugs install &lt;name&gt;</Code>,
              or let <Code>fin up</Code> install what your project needs.
            </>,
          ],
        ]}
      />
      ...
      <P>
        If you&apos;d rather not pipe a script, grab the tarball for your
        platform from the Releases page of <Code>sharanvelu/fin</Code> and place
        it yourself:
      </P>
      <CodeBlock
        lang="bash"
...
fin --help`}
      ...
      <P>
        Working on Fin itself (or on a plug) uses the Python source path — no
        prebuilt binary. This needs <strong>Python 3.11+</strong> on your PATH.
      </P>
      <CodeBlock
        lang="bash"
        prompt
...
fin --help`}
      />
      <P>
        The repo also ships a <Code>fin</Code> bash launcher at its root: it
        resolves its own real location (following symlinks), puts the package on{" "}
        <Code>PYTHONPATH</Code>, and runs <Code>python3 -m fincli</Code>{" "}
        <strong>from the directory you invoked it in</strong> — so the
        project&apos;s <Code>.env</Code> and bind mounts work correctly. Symlink
        it onto your PATH if you prefer it to the console script.
      </P>

      <H3 id="dev-plugs">Plugs for development</H3>
      <P>
        Plugs always load from the fixed <Code>~/.fin/plugs</Code> directory (
        <Code>PLUGS_DIR</Code> is not configurable independently of{" "}
        <Code>FIN_DATA_DIR</Code>). Point it at your plugs checkout once:
      </P>
      <CodeBlock
        lang="bash"
        prompt
        code={`ln -s <fin-plugs repo>/plugs ~/.fin/plugs`}
      />

      <Callout kind="warn" title="fin: command not found">
        If the installer&apos;s chosen bin directory isn&apos;t on your PATH,
        add it — e.g.{" "}
        <Code>export PATH=&quot;$HOME/.local/bin:$PATH&quot;</Code> in your
        shell profile.
      </Callout>

      <Callout kind="info" title="Conflicting docker package (source installs)">
        The source path uses the modern <Code>docker</Code> Python SDK. If you
        hit a <Code>load_config() got an unexpected keyword</Code> error, the
        abandoned <Code>docker-py</Code> package is shadowing it — run{" "}
        <Code>pip uninstall docker-py</Code> and reinstall <Code>docker</Code>.
        The prebuilt binary is unaffected.
      </Callout>
